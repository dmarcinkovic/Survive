...
void Camera::addMouseMovedListener()
{
	auto mouseMovedListener = [this](double mouseX, double mouseY) {
		...
		{
			rotation.y = m_CurrentRotation.y + dx / 2.0;
			rotation.x = m_CurrentRotation.x + dy / 2.0;
		} else if (m_RightButtonPressed)
		{
			pitch = m_CurrentPitch + static_cast<float >(dy / 5.0);
			yaw = m_CurrentYaw + static_cast<float >(dx / 5.0);
		}
	};
